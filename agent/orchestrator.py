    ...
    async def run_orchestration(self, ticket_id: str) -> dict:
        ...
        async with Agent(config=self.config) as agent:
            ...
            link_manual_resp = await agent.chat(
                f"Link the issues using the link_issues tool: inward_key: '{manual_test_key}', outward_key: '{ticket_id}', link_type_name: 'Test'."
            )
            ...
            link_cuc_resp = await agent.chat(
                f"Link the issues using the link_issues tool: inward_key: '{cucumber_test_key}', outward_key: '{ticket_id}', link_type_name: 'Test'."
            )
